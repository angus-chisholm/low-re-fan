...
def _try_load(base_dir="."):
    global RUNS, PSI_PCA, PSI_GP, PSI_LOO, ETA_PCA, ETA_GP, ETA_LOO
    if not _HAVE_CORE:
    ...
    for i, (label, (key, lo, hi, init)) in enumerate(PARAM_BOUNDS.items()):
        ...
        rows.append(html.Div([
            ...
            dcc.Slider(
                ...
                min=lo, max=hi,
                step=1 if is_int else (hi - lo) / 200,
                value=init,
...
def control_col(pfx):
    accent = C["accent_psi"] if pfx == "psi" else C["accent_eta"]
    return html.Div([
...
def audio_tab():
    return html.Div([
        html.Div([
            html.Div("🎵", style={"fontSize": "60px", "marginBottom": "12px"}),
            html.H3("Audio Analysis", style={
                "fontFamily": "'IBM Plex Mono', monospace",
                "color": C["text"], "margin": "0 0 8px 0",
            }),
            html.P("Spectral & acoustic features from DoE WAV recordings.", style={
                "fontFamily": "'IBM Plex Mono', monospace",
                "color": C["text2"], "fontSize": "13px", "marginBottom": "24px",
            }),
            html.Div([
                _feature_row("✓", "librosa loaded at startup; audio_data + sample_rate stored per-run", C["green"]),
                _feature_row("✓", f"{sum(1 for r in RUNS if r.get('audio_data') is not None)} / {len(RUNS)} runs have WAV data", C["green"]),
                _feature_row("◦", "Spectrogram viewer — coming soon", C["text3"]),
                _feature_row("◦", "MFCC / mel-band comparison across runs — coming soon", C["text3"]),
                _feature_row("◦", "Tonal noise vs broadband decomposition — coming soon", C["text3"]),
                _feature_row("◦", "1/3-octave SPL curves — coming soon", C["text3"]),
            ], style={
                "display": "inline-block", "textAlign": "left",
                "background": C["surface"],
                "border": f"1px solid {C['border']}",
                "borderRadius": "12px", "padding": "20px 28px",
                "boxShadow": f"0 2px 12px {C['shadow']}",
            }),
...
app.layout = html.Div([
    ...
    dcc.Tabs(
        ...
        children=[
            ...
            dcc.Tab(label="Tab 3 — 🎵  Audio  (coming soon)",  value="audio", className="tab"),
        ],
...
def render_tab(tab):
    if tab == "psi":   return tab_panel("psi")
    if tab == "eta":   return tab_panel("eta")
    return audio_tab()
...
#  Core plot builder
# ─────────────────────────────────────────────────────────────────────────────
def _build_plots(pfx, slider_vals, sweep_idx, n_modes):
    is_psi    = pfx == "psi"
...
def update_psi(slider_vals, sweep, nmodes):
    if not slider_vals:
        e = _empty()
        return e, e, e, "Waiting for sliders…"
    try:
        return _build_plots("psi", slider_vals, sweep, nmodes or N_MODES_INIT)
    except Exception as exc:
...
def export_psi(_, vals, nmodes):
    return _do_export("psi", vals, nmodes or N_MODES_INIT)
...
def update_eta(slider_vals, sweep, nmodes):
    if not slider_vals:
        e = _empty()
        return e, e, e, "Waiting for sliders…"
    try:
        return _build_plots("eta", slider_vals, sweep, nmodes or N_MODES_INIT)
    except Exception as exc:
        e = _empty(f"Error: {exc}")
        return e, e, e, str(exc)
...
def export_eta(_, vals, nmodes):
    return _do_export("eta", vals, nmodes or N_MODES_INIT)


# ─────────────────────────────────────────────────────────────────────────────
#  CSV export
# ─────────────────────────────────────────────────────────────────────────────
def _do_export(pfx, slider_vals, n_modes):
    is_psi    = pfx == "psi"
    pca_obj   = PSI_PCA if is_psi else ETA_PCA
    gp_obj    = PSI_GP  if is_psi else ETA_GP
    curve_key = "psi" if is_psi else "eta"
