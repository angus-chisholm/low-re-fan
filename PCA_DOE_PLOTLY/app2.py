...
def audio_tab():
    return html.Div([
        control_col("audio"),
        html.Div([
            # OASPL badge sits above the main chart
            html.Div([
            ...
            dcc.Graph(id="audio-main", config={"displayModeBar": True, "displaylogo": False,
...
#  Audio plot builder
# ─────────────────────────────────────────────────────────────────────────────
def _build_audio_plots(slider_vals, sweep_idx, n_modes):
    global AUDIO_PCA, AUDIO_GP, AUDIO_LOO

    accent = C["accent_audio"]
    band1  = C["band_audio"]
    band2  = C["band_audio2"]

    if AUDIO_PCA is None or AUDIO_PCA.n_modes != n_modes:
        if not RUNS:
            return _empty(), _empty(), _empty(), "—", "No data loaded."
    ...
    if sweep_idx >= 0:
        ...
        for i, val in enumerate(s_vals):
            pv = list(param_vals); pv[sweep_idx] = val
            sm, _, _, _, _, _ = AUDIO_GP.predict(pv)
            yy  = AUDIO_PCA.reconstruct(sm)
            hue = int(170 + i * 16)
            lum = int(28 + i * 6)
            col = f"hsl({hue}, 68%, {lum}%)"
            fig.add_trace(go.Scatter(
    ...
    fig.update_layout(
        **PLOT_LAYOUT_BASE,
        ...
        xaxis=dict(
            ...
            title="Frequency  [Hz]",
            tickformat=".0f",
            minor=dict(ticks="inside", showgrid=True, gridcolor=C["border"]),
        ),
    ...
    k_range = list(range(1, k_max + 1))
    ind_var = AUDIO_PCA.var_explained[:k_max].tolist()
    cum_var = AUDIO_PCA.cum_var[:k_max].tolist()

    fig_sc = go.Figure()
    fig_sc.add_trace(go.Bar(
        x=k_range, y=ind_var,
        marker_color=accent, opacity=0.55, name="Individual %",
    ))
    fig_sc.add_trace(go.Scatter(
        x=k_range, y=cum_var, mode="lines+markers",
        line=dict(color=C["amber"], width=2),
...
def update_audio(slider_vals, sweep, nmodes):
    if not slider_vals:
        e = _empty()
        return e, e, e, "—", "Waiting for sliders…"
    try:
        return _build_audio_plots(slider_vals, sweep or -1, nmodes or N_MODES_INIT)
    except Exception as exc:
        e = _empty(f"Error: {exc}")
        return e, e, e, "—", str(exc)
...
def export_audio(_, vals, nmodes):
    if AUDIO_PCA is None or AUDIO_GP is None:
        return None, "⚠ No model — load data first."
    n_modes   = nmodes or N_MODES_INIT
    freqs     = np.array(CENTRE_FREQS)
    scores_m, _, _, _, _, _ = AUDIO_GP.predict(list(vals))
    pred      = AUDIO_PCA.reconstruct(scores_m)
    oaspl_val = calculate_oaspl(pred)
    buf = io.StringIO()
    w   = csv.writer(buf)
